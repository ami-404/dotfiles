...
while true; do
    ...
    *"Manual Entry")
        ...
        if [ -z "$wifi_password" ]; then
            output=$(pkexec nmcli device wifi connect "$manual_ssid" hidden yes 2>&1)
        else
            output=$(pkexec nmcli device wifi connect "$manual_ssid" password "$wifi_password" hidden yes 2>&1)
        fi
        ...
        if echo "$output" | grep -q "successfully"; then
            ...
            notify-send "Failed: $output" -i "package-broken"
        fi
        ;;
    *)
        ...
        if echo "$saved_connections" | grep -qw "$selected_ssid"; then
            output=$(nmcli connection up id "$selected_ssid" 2>&1)
        else
            if [[ "$selected_option" =~ ^"󰤪" ]]; then
                wifi_password=$(get_password)
            fi
            output=$(pkexec nmcli device wifi connect "$selected_ssid" password "$wifi_password" 2>&1)
        fi
