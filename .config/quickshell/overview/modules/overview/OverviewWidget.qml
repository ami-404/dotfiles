                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    if (root.draggingTargetWorkspace === -1) {
                                        ...
                                        Quickshell.execDetached(["hyprctl", "dispatch", `hl.dsp.focus({ workspace = "${workspaceValue}" })`])
                                    }
                                }
                            }
                    ...
                    MouseArea {
                        ...
                        onReleased: {
                            ...
                            if (targetWorkspace !== -1 && targetWorkspace !== windowData?.workspace.id) {
                                ...
                                Quickshell.execDetached(["hyprctl", "dispatch", `hl.dsp.window.move({ workspace = "${targetWorkspace}", window = "address:${window.windowData?.address}" })`])
                        ...
                        onClicked: (event) => {
                            ...
                            if (event.button === Qt.LeftButton) {
                                ...
                                Quickshell.execDetached(["hyprctl", "dispatch", `hl.dsp.focus({ window = "address:${windowData.address}" })`])
                                event.accepted = true
                            } else if (event.button === Qt.MiddleButton) {
                                // Hyprland.dispatch(`closewindow address:${windowData.address}`)
                                Quickshell.execDetached(["hyprctl", "dispatch", `hl.dsp.window.close({ window = "address:${windowData.address}" })`])
                                event.accepted = true
                            }
                        }
